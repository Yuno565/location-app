...
app.get("/api/reservations", (req, res) => {
  db.query("SELECT * FROM reservations ORDER BY pickup_date DESC", (err, results) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json(results);
  });
});
...
app.post("/api/reservations", (req, res) => {
  ...
  const sql = `INSERT INTO reservations 
    (id, vehicle_id, vehicle_name, customer_name, customer_email, customer_phone, pickup_city, pickup_date, return_date, total_days, total_price, payment_method, status) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
